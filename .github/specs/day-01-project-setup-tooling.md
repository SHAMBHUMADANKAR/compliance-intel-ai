...
- **AI ecosystem gravity.** LangChain, LangGraph, Hugging Face `transformers`, `sentence-transformers`, and `qdrant-client` are all Python-native. Picking a Python backend means zero translation layer between "the web framework" and "the AI code" — they're the same process, same imports, same virtualenv.
...
- Compare: Django is batteries-included but opinionated in ways (ORM, ORM-coupled admin) that fight a system where Qdrant, not Postgres, is doing a lot of the "querying." FastAPI is a thin, unopinionated layer — you bring your own DB layer (Day 2), your own auth (Day 3).
